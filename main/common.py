...
class Node:
    ...
    ----------------------------------------------------------------------'''
    def __init__(self, m, position, momentum, type, color):
        '''-------------------------------------------------------
...
def galaxy_init(N, max_mass, BHM, center, BHmomentum, ini_radius, alpha, beta, color):
    '''-------------------------------------------------------------------
    Initializes the N-body galaxy by defining the position and momentum
    of the bodies and creating the corresponding objects of the Node class
    --------------------------------------------------------------------'''
    #Defines initial conditions
    bodies = []
    bodies.append(Node(BHM, position=center, momentum=BHmomentum, type='BH', color='lightcyan'))   
    masses, positions, momenta = spiral_galaxy(N, max_mass, BHM, center, ini_radius, alpha, beta)
    #Creates nodes
    for i in range(N-1):
        bodies.append(Node(masses[i], positions[i], momenta[i], 'body', color))
    
    return bodies

def evolve(bodies, n, img_step, image_folder='images/'):
    '''---------------------------------------------------------------------------------------------
    ...
    for i in range(n+1):
        ...
        if i%img_step==0:
            print("Writing image at time {0:06}".format(i//img_step))
...
def plot_bodies(bodies, i, image_folder='images/'):
    ...
    axa = plt.gcf().add_subplot(321, projection='3d')
    axa.set_xlim([0,1.5])
    axa.set_ylim([0,1.5])
    axa.set_zlim([0,1.5])
    axa.set_facecolor('black')
    axa.xaxis.pane.fill = False
    axa.yaxis.pane.fill = False
    axa.zaxis.pane.fill = False
    axa.view_init(90, -90)

    ax = plt.gcf().add_subplot(323, projection='3d')
    ax.set_xlim([0,1.5])
    ax.set_ylim([0,1.5])
    ax.set_zlim([0,1.5])
    ...
    ax2 = plt.gcf().add_subplot(324, projection='3d')
    ax2.set_xlim([0,1.5])
    ax2.set_ylim([0,1.5])
    ax2.set_zlim([0,1.5])
    ...
    ax3 = plt.gcf().add_subplot(325, projection='3d')
    ax3.set_xlim([0,1.5])
    ax3.set_ylim([0,1.5])
    ax3.set_zlim([0,1.5])
    ...
    for body in bodies:
        pos = body.position()
        z_order = 1 if body.type == 'BH' else 2
        s = 20*4*2 if body.type == 'BH' else 20
        axa.scatter(pos[0], pos[1], pos[2], marker='.', color=body.color, zorder=z_order, s = s)
        ax.scatter(pos[0], pos[1], pos[2], marker='.', color=body.color, zorder=z_order, s = s)
        ax2.scatter(pos[0], pos[1], pos[2], marker='.', color=body.color, zorder=z_order, s = s)
        ax3.scatter(pos[0], pos[1], pos[2], marker='.', color=body.color, zorder=z_order, s = s)
